...
//! instance — a `MapsView` root minted there, and the host's teardown
//! order: the refs go first, then the isolate. After News' own.

use crate::module::MAPS_MODULE;
...
impl Isolate {
    ...
    /// `MapsModule::register` puts them there for the host.
    pub(crate) fn new() -> Self {
        let mut cx = Cx::new(Box::new(|_, _| {}));
        cx.init_cx_os();
        cx.with_vm(makepad_widgets::script_mod);
        let vm_id = cx.alloc_splash_vm_with_network(false);
        cx.with_script_vm_id_trusted(vm_id, |vm| MAPS_MODULE.register(vm));
        Isolate { cx, vm_id }
    }
...
/// A `MapsView {}` root minted in a fresh isolate, which evaluated it
...
pub(crate) fn isolate_root() -> (Isolate, WidgetRef) {
    ...
    let root = iso.with_vm(|vm| {
        ...
        assert!(
            errors.is_empty(),
            "the isolate evaluated the view without errors: {errors:?}"
        );
        root
    });
    (iso, root)
